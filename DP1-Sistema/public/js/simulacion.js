...
AmCharts.ready(function() {
  ...
  map.legend={
     ...
     horizontalGap: 10,
      data: [ {
      title: "Nivel bajo (0-30)%",
      color: "#000000"
    },{
      title: "Nivel medio (30-70)%",
      color: "#ebdb8b"
    }, {
      title: "Nivel alto (70-100)%",
      color: "#db8383"
    }]
    };
...
function updateLog(wsEvent) {
  ...
  if (wsEvent.data == "1"){
    ...
    if (wsEvent.data == "0"){
      ...
      var jsonCaps=JSON.parse(wsEvent.data);
      actualizarCapsCiudades(jsonCaps.capsCiudad);
      writeToScreen("<span style='color: blue'>Received: " + jsonCaps.keyCiudad + "</span>");
      writeToScreen("<span style='color: blue'>Received: " + jsonCaps.capsCiudad + "</span>");
    }
...
function actualizarCapsCiudades(caps){
  ...
  for (var i = 0; i < rowLength; i++) {
    ...
    if(caps[i]>420)  {
      ...
      map.dataProvider.images[i].color="#ebdb8b";
      map.dataProvider.images[i].scale=1.5;
    }  
    else {
      map.dataProvider.images[i].color="#000000";
      map.dataProvider.images[i].scale=0.5;
    }  
  }
}
